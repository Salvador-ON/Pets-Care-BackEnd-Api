class Appointment < ApplicationRecord
  ...
  appointments_select = 'appointments.id, appointments.date, appointments.pet_name, appointments.time,'
  users_select = ' users.name, users.phone, users.id as user_id, services.name as service_name '

  scope :mine, ->(cu_uid) { where(user_id: cu_uid).joins(:user).joins(:service).select(appointments_select + users_select) }
